...
static __global__ void quantize_mmq_q8_1(
        ...
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne0, const int ne1, const int ne2, const int n_expert_used, const float * gate = nullptr) {
    ...
    if constexpr (swiglu) {
        if (i0 < ne00) {
            ...
            xi.x = ggml_cuda_op_silu_single(xi.x) * gi.x;
            xi.y = ggml_cuda_op_silu_single(xi.y) * gi.y;
            xi.z = ggml_cuda_op_silu_single(xi.z) * gi.z;
            xi.w = ggml_cuda_op_silu_single(xi.w) * gi.w;
        }
    }
...
void quantize_swiglu_mmq_q8_1_cuda(
        const float * x, const float * gate, void * vy, ggml_type type_src0,
        int64_t ne00, int64_t ne0, int64_t ne1, cudaStream_t stream) {
    GGML_ASSERT(ne00 % 4 == 0);
    GGML_ASSERT(ne0 % QK8_1_MMQ == 0);
    const int64_t block_num_y = (ne0 + 4*CUDA_QUANTIZE_BLOCK_SIZE_MMQ - 1) / (4*CUDA_QUANTIZE_BLOCK_SIZE_MMQ);
    ...
    switch (mmq_get_q8_1_ds_layout(type_src0)) {
        case MMQ_Q8_1_DS_LAYOUT_D4:
            quantize_mmq_q8_1<MMQ_Q8_1_DS_LAYOUT_D4, false, true>
                <<<num_blocks, block_size, 0, stream>>>(x, nullptr, vy, ne00, ne00, 0, 0, ne0, ne1, 1, 0, gate);
            break;
        case MMQ_Q8_1_DS_LAYOUT_DS4:
